...
function init() {
  ...
  const TorusGeometry = new THREE.TorusBufferGeometry(120, 10, 2.3, 100);
  const TorusMaterial = new THREE.MeshLambertMaterial({ color: 0xcccccc });
  const TorusMesh = new THREE.Mesh(TorusGeometry, TorusMaterial);
  TorusMesh.rotation.x = Math.PI / 2;
  TorusMesh.rotation.y = -0.1 * (Math.PI / 2);
  scene.add(TorusMesh);

  const meshList = [];
  const IcoGeometry = new THREE.IcosahedronGeometry(10, 0);
  const IcoMaterial = new THREE.MeshToonMaterial({ color: 0x5eb1e0 });
